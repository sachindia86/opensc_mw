										...
										u8 * out, size_t outlen)
{
	...
	u8* p=sbuf;
	size_t size = datalen; // commented by Neel for testing
	//size_t size = 2; // commented by Neel for testing
	...
	memcpy(p,data,size); // commented by Neel for testing
...
/*	if(!card->drv_data)
   ...
   */
	...
	apdu.data=p;
	apdu.lc = 255; // Changed to 2 (from 255) by Neel for testing
	apdu.datalen = 255; // Changed to 2 (from 255) by Neel for testing
	apdu.resp = rbuf;
	apdu.resplen = sizeof(rbuf);
	apdu.le = 256;
